...
async def process_cmd(c, m):
    ...
    if FREEMIUM_LIMIT == 0 and not await is_premium_user(uid):
        await m.reply_text("此机器人不提供免费服务,请从管理员处获取订阅")
        return

    if await sub(c, m) == 1: return
    pro = await m.reply_text('正在做一些检查,请稍候...')
    
    if is_user_active(uid):
        await pro.edit('您有一个正在进行的任务。使用 /stop 取消它。')
        return
...
async def cancel_cmd(c, m):
    ...
    if is_user_active(uid):
        if await request_batch_cancel(uid):
            await m.reply_text('已请求取消。当前批量任务将在当前下载完成后停止。')
        else:
            await m.reply_text('请求取消失败。请再试一次。')
    else:
        await m.reply_text('未找到活动的批量任务。')
...
async def text_handler(c, m):
    ...
    if not x:
        await message.reply("请使用 /setbot `token` 添加您的机器人")
        return
    ...
    if s == 'start':
        ...
        if not uc or not ubot:
            await pt.edit('缺少客户端设置')
            Z.pop(uid, None)
            return

        if is_user_active(uid):
            await pt.edit('存在活动任务')
            Z.pop(uid, None)
            return

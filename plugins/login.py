...
async def set_bot_token(C, m):
    ...
    if len(args) < 2:
        await m.reply_text("⚠️ 请提供机器人令牌。用法: `/setbto token`", quote=True)
        return
...
async def rem_bot_token(C, m):
    ...
    await m.reply_text("✅ 机器人令牌已成功删除。", quote=True)
...
async def cancel_command(client, message):
    ...
    if get_user_step(user_id):
        ...
        temp_msg = await message.reply('没有活动的登录流程需要取消。')
        await temp_msg.delete(5)
